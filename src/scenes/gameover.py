...
class GameOver:
    ...
    def draw_multiline_text(self, surface, text, font, color, x, y, max_width, line_spacing=1.2):
    ...
    def run(self, game, events):
        ...
        # Ajustar tamaños de fuente según el nivel (algunos títulos son más largos)
        if self.level_id == "3":
            title_size = int(base_size * 0.06)  # Más pequeño para títulos largos
        else:
        ...
        # Obtener textos
        title_text = get_text(self.translations, game.current_lang, f"gameover-title-{self.level_id}")
        subtitle_text = get_text(self.translations, game.current_lang, f"gameover-subtitle-{self.level_id}")
        menu_text = get_text(self.translations, game.current_lang, "press-m-to-menu")
        restart_text = get_text(self.translations, game.current_lang, "press-r-to-restart")

        # Anchos máximos (con márgenes)
        ...
        subtitle_lines = self.wrap_text(subtitle_text, subtitle_font, max_subtitle_width)
        ...
        actions_height = menu_font.get_linesize() * 3  # Espacio para ambas opciones
        ...
        # Posición vertical inicial (centrado)
        start_y = (self.window_height - total_height) / 2
        current_y = start_y

        # DIBUJAR TÍTULO
        title_height_used = self.draw_multiline_text(
            self.game_screen, title_text, title_font, "white", 
            self.window_width / 2, current_y, max_title_width, 1.3
        )
        current_y += title_height_used + spacing

        # DIBUJAR SUBTÍTULO
        subtitle_height_used = self.draw_multiline_text(
            self.game_screen, subtitle_text, subtitle_font, "white", 
            self.window_width / 2, current_y, max_subtitle_width, 1.3
        )
        ...
        restart_surface = menu_font.render(restart_text, True, "white")
        restart_rect = restart_surface.get_rect(center=(self.window_width / 2, current_y))
